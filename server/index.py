...
def handler(event, context):
    try:
        ...
        if protocol == "socks5":
            return handle_socks5_request(data)

        return handle_http_request(data)
    ...
    if not url:
        ...
        if "body" in data:
            ...
            if isinstance(body_value, (bytes, bytearray)):
                ...
                body_text = repr(body_value)
            debug["body_preview"] = body_text[:300]
...
def handle_socks5_request(data):
    try:
        ...
        if not host:
            return mk_response(400, {"error": "Missing host for SOCKS5 request"})

        if command == 0x01:
            return handle_socks5_connect(host, port, connect_data)

        return mk_response(400, {"error": f"Unsupported SOCKS5 command: {command}"})

    except Exception as exc:
        return mk_response(502, {"error": f"SOCKS5 Error: {exc}"})


def handle_socks5_connect(host, port, connect_data):
    try:
        ...
        if connect_data:
            if connect_data.startswith("data:base64,"):
                encoded = connect_data.split("data:base64,", 1)[1]
                decoded = base64.b64decode(encoded)
                sock.sendall(decoded)
            else:
                sock.sendall(connect_data.encode("utf-8"))
        ...
        return mk_response(200, {
            "status_code": 200,
            "headers": {},
            "content": base64.b64encode(response_data).decode("utf-8"),
            "is_content_base64": True,
        })

    except socket.error as exc:
        return mk_response(502, {"error": f"SOCKS5 Connect Error: {exc}"})
    except Exception as exc:
        return mk_response(502, {"error": f"SOCKS5 Error: {exc}"})
