...
func (s *ProxyServer) startSocks5Proxy() {
	// 配置认证（如果设置了用户名密码）
	var authOpts []socks5.Authenticator
	if s.User != "" && s.Password != "" {
		authOpts = append(authOpts, socks5.UserPassAuthenticator{
		...
		// 允许无认证
		authOpts = append(authOpts, socks5.NoAuthAuthenticator{})
	}

	conf := &socks5.Config{
		AuthMethods: authOpts,
		Dial: func(ctx context.Context, network, addr string) (net.Conn, error) {
			if s.Verbose {
				log.Printf("[SOCKS5] Dial: %s://%s", network, addr)
			}
			// 统一通过 CONNECT 桥接到本地 HTTP 代理
			// HTTP 代理会处理 MITM 和云函数转发
	...
	log.Printf("SOCKS5 代理服务正在监听: %s (认证: %v)", s.SocksAddr, s.User != "")
