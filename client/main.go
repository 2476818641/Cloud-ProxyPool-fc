...
func main() {
	...
	if len(args) > 0 && args[0] == "auth" {
		handleAuthCommand(args[1:], configPath)
		return
	}
	...
	if conf.Client.DashboardAddr != "" {
		go dashboard.StartDashboard(conf.Client.DashboardAddr, srv, provider, configPath)
		provider.SetNodeEnabledFunc(dashboard.IsNodeEnabled)
	}
